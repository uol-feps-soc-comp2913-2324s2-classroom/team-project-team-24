<template>
    <div class="myGroupPageContainer d-flex flex-column">
        <div
            class="groupViewHeading d-flex flex-row justify-content-between align-items-center p-3"
            id="gropViewHeadingContainer"
        >
            <div class="d-flex flex-row align-items-center groupName">
                <img
                    src="../assets/back_button.svg"
                    class="backButton me-3"
                    alt="back arrow icon"
                    @click="returnToCommunity"
                />
                <h3 v-if="!loadingGroupName" class="groupNameText">
                    {{ name }}
                </h3>
            ...
            <div class="d-flex flex-row">
                ...
                <button
                    @click="inviteFriends"
                    class="btn-secondary btn-desktop"
                >
                    Group&nbsp;members
                </button>
                ...
                <button @click="inviteFriends" class="btn-secondary btn-mobile">
                    Members
                </button>
            </div>
            ...
            >
                <div class="addTrailsModalWindow d-flex flex-column">
                    <topNavRailedGroupTrails
                        @NavElementClicked="handleNavElementClickedTrails"
                        class="mb-3"
                        :lastSelectedElement="lastTrailsModalView"
                    />
                    <div v-if="trailsModalShowSharedTrails" class="mb-4">
                        <div class="scrollable mb-4">
                            <ListComponent
                                v-bind:dataArray="groupTrails"
                                v-slot="slotProps"
                                v-if="groupTrails.length > 0"
                                class="width100"
                            >
                                <SharedTrailListItemComponent
                                    v-bind:trail="slotProps.data"
                            ...
                            <p
                                ...
                                class="greyText"
                            >
                                You haven't shared any trails yet...
                            </p>
                            <p v-if="loadingGroupTrails" class="greyText">
                                Loading shared trails...
                            </p>
                        </div>
                    </div>
                    <div v-if="trailsModalShowAddTrails">
                        <div
                            class="scrollableList mb-4"
                            v-show="!loadingTrailList && trailItemLoaded"
                        >
                            <ListComponent
                                v-bind:dataArray="shareableTrails"
                                v-slot="slotProps"
                                v-if="shareableTrails.length > 0"
                                @listComponentUnmounted="
                            ...
                            >
                                <AddTrailListItemComponent
                                    ...
                                    @trailItemDataUpdated="
                                        handleTrailItemDataUpdated
                                    "
                                    @trailAddedToGroup="updateTrailsList"
                                />
                            </ListComponent>
                            <p
                                v-if="shareableTrails.length == 0"
                                class="greyText align-self-start"
                            >
                                No trails available
                            </p>
                        </div>
                        <p
                            v-if="loadingTrailList || !trailItemLoaded"
                            class="greyText mb-4"
                        >
                            Loading trails...
                        </p>
                    ...
                    <button
                        @click="closeTrailsPopup"
                        class="btn-secondary align-self-end mt-2"
                    >
                        Close
                    </button>
                </div>
            ...
            <ModalComponent
                :is-open="showFriends"
                @update:is-open="showFriends = $event"
                :lastSelectedElement="lastGroupMembersModalView"
            >
                <div class="groupMembersModalWindow d-flex flex-column">
                    <topNavRailedGroupMembers
                        @NavElementClicked="handleNavElementClicked"
                        class="mb-3"
                    />
                    <div v-if="groupModalShowMembers">
                        <div class="scrollableList mb-4">
                            <div
                                style="display: flex"
                                class="friendItem slightlySmaller"
                            >
                    ...
                    <div v-if="groupModalShowAddMembers" class="mb-4">
                        <div class="scrollableList" v-if="!loadingFriendsList">
                            <ListComponent
                                v-bind:dataArray="friendsNonMembers"
                                v-slot="slotProps"
                                v-if="friendsNonMembers.length > 0"
                                class="width100"
                            >
                                <UserListItemComponent
                                    v-bind:user="slotProps.data"
                        ...
                        <p v-if="loadingFriendsList" class="greyText">
                            Loading friends...
                        </p>
                    </div>
                    <button
                        @click="closeFriendsPopup"
                        class="btn-secondary align-self-end mt-2"
                    >
                        Close
                    </button>
        ...
        <div
            class="groupMapView"
            id="mapViewComponent"
            :style="mapViewComponentStyleForce"
        >
            <MapViewerComponent
                :selected-trails="groupTrails.map((trail) => trail.id)"
                :width="mapWidth"
                :height="mapHeight"
            />
        </div>
...
export default {
    ...
    methods: {
        ...
        async getPageData() {
            ...
            const getMembersPromise = axiosAuth
                .post('/groups/get-members', {
            ...
            const getAllFriendsPromise = axiosAuth
                .get('/friends/get-all')
                .then((response) => {
                    this.friends = response.data.friends
                })

            const getAllTrailsPromise = axiosAuth
                .get('/trail/get-all')
                .then((response) => {
                    this.trails = response.data.trails
                })

            const getCurrentUsernamePromise = axiosAuth
            ...
            const getGroupTrailsPromise = axiosAuth
                .post('groups/get-trails-complete', {
                    groupID: this.groupID,
                })
                .then((response) => {
            ...
            await Promise.all([
                getMembersPromise,
                getAllFriendsPromise,
                getCurrentUsernamePromise,
            ]).then(() => {
            ...
            await Promise.all([
                getAllTrailsPromise,
                getGroupTrailsPromise,
            ]).then(() => {
                const groupTrailsSet = new Set(
                    this.groupTrails.map((trail) => trail.id)
                )
                this.shareableTrails = this.trails.filter(
                    (trail) => !groupTrailsSet.has(trail.id)
                )
        ...
        leaveGroup() {
            axiosAuth
                ...
                .then(this.$router.push({ path: '/community' }))
        },
        closeFriendsPopup() {
        ...
        handleTrailItemDataUpdated() {
            ...
            if (
                this.trailItemFullyLoadedCount === this.shareableTrails.length
            ) {
                this.trailItemLoaded = true
            }
        },
