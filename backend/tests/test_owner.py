...
def test_get_owner_membership_data_success(client):
    ...
    assert response.json["numUsers"] == 4
    assert {"id": m1.id, "name": m1.name, "payment_regularity": m1.payment_regularity, "cost": m1.cost, "numMembers": 2} in response.json["memberships"]
